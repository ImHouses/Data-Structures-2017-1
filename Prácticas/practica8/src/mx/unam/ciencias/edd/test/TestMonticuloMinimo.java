...
public class TestMonticuloMinimo {
    ...
    @Test public void testEquals() {
        Assert.assertFalse(monticulo.equals(null));
        MonticuloMinimo<Indexable<String>> otro =
            new MonticuloMinimo<Indexable<String>>();
        Assert.assertFalse(monticulo.equals(otro));
        for (int i = 0; i < total; i++) {
            Indexable<String> idx = monticulo.get(i);
            otro.agrega(new Indexable<String>(idx.getElemento(),
                                              idx.getValor()));
        }
        Assert.assertTrue(monticulo.equals(otro));
        Indexable<String> idx = monticulo.elimina();
        Assert.assertFalse(monticulo.equals(otro));
        idx.setValor(idx.getValor() * 2.0);
        monticulo.agrega(idx);
        Assert.assertFalse(monticulo.equals(otro));
        Assert.assertFalse(monticulo.equals(""));
        Assert.assertFalse(monticulo.equals(null));
    }

...
public class Lista<T> implements Coleccion<T> {
    ...
    Lista<T> mergeSort(Lista<T> l) {
        ...
        for (int i = 0;i < l.longitud / 2 ;i++ ) {
            izq.agregaFinal(n.elemento);
            n = n.siguiente;            
        }
        for (int i = l.longitud / 2;i < l.longitud; i++ ) {
            der.agregaFinal(n.elemento);
            n = n.siguiente;
        }
        return merge(mergeSort(izq), mergeSort(der));
    }
    private static <T extends Comparable<T>> Lista<T> merge(Lista<T> izq, Lista<T> der) {
        Lista<T> res = new Lista<T>();
        Lista<T>.Nodo i = izq.cabeza;
        Lista<T>.Nodo j = der.cabeza;
        if (der.esVacio())
            return izq;
        else if(izq.esVacio())
            return der;
        while (i != null && j != null) {
            if (i.elemento.compareTo(j.elemento) <= 0) {
                res.agregaFinal(i.elemento);
                i = i.siguiente;
            }else {
                res.agregaFinal(j.elemento);
                j = j.siguiente;
            }
        }
        while (i != null) {
